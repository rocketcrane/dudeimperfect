...
function paintBoard() {
  ...
  world.proj.display();  // ball
  rectMode(CENTER);
  rect((world.x*scale_to_screen), (-world.y*scale_to_screen), 40, 4); // hoop

  rectMode(CORNERS);
  rect(axis_dis, axis_dis_2, (world.x*scale_to_screen), (axis_dis_2+axis_width)); // x axis
  text(world.x+" m", (world.x/2)*scale_to_screen, 13); // x label
  rect((-axis_dis_2), (-axis_dis), (-axis_dis_2+axis_width), (-world.y*scale_to_screen)); //y axis
  rotate(radians(270));
  text(world.y+" m", (world.y/2)*scale_to_screen, -30); // y label
}
