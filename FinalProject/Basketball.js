...
function shoot() {
  ...
  if (swingTimer > 60) {
    image(imgBB, hooploc - distance + robotB.width/3, maxY - robotB.height/2);
  }
  if (swingTimer > 0) {
    swingTimer--;
  }
...
function bbVisuals() {
  ...
  if (win) {
    image(robotBWin, hooploc - distance, maxY - robotG.height/2);
  } else if (swingTimer > 20 && swingTimer < 60) {
    image(robotBShoot, hooploc - distance, maxY - robotG.height/2);
  } else if (swingTimer > 0) {
    image(robotB, hooploc - distance, maxY - robotG.height/2);
  } else {
    image(robotB, hooploc - distance, maxY - robotG.height/2);
  }
  ...
  if (!win) {
    ...
    fill(255);
    if (level == 1) {
      text("Time: 0.91s", width/2 + 50, height/4 + 120);
    }
    if (level == 2) {
      text("Time: 1.36s", width/2 + 50, height/4 + 120);
    }
    if (level == 3) {
      text("Time: 1.70s", width/2 + 50, height/4 + 120);
    }
    if (level == 4) {
      text("Time: 2.22s", width/2 + 50, height/4 + 120);
    }
  }
  if (!win) {
