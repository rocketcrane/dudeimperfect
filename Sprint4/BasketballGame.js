class BasketballGame extends Game {
  constructor(grav, time_force_applied, canonicalDist) {
  ...
  updateConstants() {
    ...
    this.DISTANCE = width * 6/8;
    useAsHeightReference(this.DISTANCE, this.canonicalDist);
    this.PLAYERWIDTH = canonicalToActual(0.6);
    this.PLAYERHEIGHT = canonicalToActual(1.8);
    this.BALLSIZE = canonicalToActual(0.3);
    this.BALLHEIGHT = this.PLAYERHEIGHT + canonicalToActual(0.1);
    this.HOOPSIZE = canonicalToActual(1.5);
    this.HOOPHEIGHT = this.HOOPSIZE + canonicalToActual(2.4);
    this.RIMHEIGHT = this.HOOPHEIGHT - this.HOOPSIZE/1.75;

    // distance to goal
    drawHorizontalDist(FLOOR + 20, WALL, WALL + this.DISTANCE);
    ...
    if (IS_MOVING) {
      if (this.SHOOTINGTIMER <= 60) {
        this.SHOOTINGTIMER++;
        drawImage(imgBasketballPlayerShoot, WALL, this.PLAYERHEIGHT);
      } else {
        drawImage(imgBasketballPlayerStill, WALL, this.PLAYERHEIGHT);
      }
      this.proj.move();
    } else {
      drawImage(imgBasketballPlayer, WALL, this.PLAYERHEIGHT);
    }
    drawVerticalDist(width/5, FLOOR, FLOOR - this.BALLHEIGHT);

    // update hoop
    drawImage(imgBasketballHoop, WALL + this.DISTANCE, this.HOOPHEIGHT);
    drawVerticalDist(width - width/4, FLOOR, FLOOR - this.RIMHEIGHT);
  }
